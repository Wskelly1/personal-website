...
export default function TopReadsCarousel({ books, onBookClick }: TopReadsCarouselProps) {
  const [index, setIndex] = useState(0);
  const visibleCount = 3; // Show three books at a time
  const maxIndex = Math.max(0, books.length - visibleCount);
  const canPrev = index > 0;
  const canNext = index < maxIndex;

  return (
    <div className="relative w-[48rem] min-h-[26rem] flex flex-col items-center justify-center">
      <div className="flex items-center relative">
        ...
        <div className="flex gap-6 overflow-hidden w-[48rem]">
          {books.slice(index, index + visibleCount).map((item) => (
            <div
              key={item._id}
              className="group relative w-60 aspect-[2/3] rounded-lg overflow-hidden flex-shrink-0 cursor-pointer border-4 border-[#e9f1ff] dark:border-[#5f6c89]"
